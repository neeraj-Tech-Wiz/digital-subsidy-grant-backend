...
public class EligibilityEvaluationService {
    ...
    public EligibilityEvaluationResult evaluate(Scheme scheme, EligibilityEvaluationRequest request) {
        if (request == null) {
        ...
        BeneficiaryProfile profile = new BeneficiaryProfile(
                ...
                request.getLandHoldingAcres(),
                request.getCategory(),
                request.getRegion()
        );
        profile.setAadhaarVerified(request.isAadhaarVerified());
        profile.setMobileVerified(request.isMobileVerified());
        profile.setDocumentsVerified(request.isDocumentsVerified());
        profile.setPreviousBenefitsReceived(request.isPreviousBenefitsReceived());
        if (request.getAttributes() != null) {
            profile.setAttributes(request.getAttributes());
        }

        return evaluate(scheme, profile);
    }
